...
const DailyRotateFileTransport = require('zs-winston-daily-rotate-file');
 ...
 *   @param {String} [config.logDirectory='./log/'] - Base directory for logs.
...
class LogServer extends winstond.nssocket.Server {
	...
	newLineFormatter(options) {
		if (options.message === 'undefined') { delete options.message; }
		let ret = `timestamp: ${(new Date()).toISOString()}${os.EOL}level: ${options.level}${os.EOL}`;
	...
	humanReadableFormatter(options) {
		...
		_.extend(logObj, { timestamp: (new Date()).toISOString() });
	...
	jsonFormatter(options) {
		...
		_.extend(ret, { timestamp: (new Date()).toISOString() });
...
function _genAllTransports(dir, keepDays = {}) {
	...
	transports.push(_genDailyRotateTransport('main.log', dir, 'silly', keepDays.main, LogServer.prototype.newLineFormatter));
