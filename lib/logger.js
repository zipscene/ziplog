...
class Logger {
	 ...
	 *
	 * @method _convertError
	 * @private
	 * @static
	 * @param {Error} err
	 * @return {Object}
	 */
	static _convertError(err) {
		...
		for (let prop in err) {
			if (prop === 'cause') {
				ret[prop] = Logger._convertError(err[prop]);
			} else {
				ret[prop] = err[prop];
			}
		}
		// These properties are not enumerable in several node versions
		// https://gist.github.com/indexzero/1553662
		for (let prop of [ 'message', 'stack' ]) {
			if (err[prop]) ret[prop] = err[prop];
		}
		return ret;
	}
	...
	_makeLogEntry(fields = {}, ...args) {
		...
		for (let arg of args) {
			if (_.isString(arg)) {
				if (_.includes([ 'silly', 'debug', 'verbose', 'info', 'warn', 'error' ], arg)) {
					if (!result.level) {
						result.level = arg;
					} else {
				...
				if (arg instanceof Error) {
					arg = Logger._convertError(arg);
					if (firstError) {
						if (!result.details) result.details = {};
						result.details.error = arg;
