...
class Logger {
	...
	_makeLogEntry(fields = {}, ...args) {
		...
		if (args.length === 1 && _.isObject(args[0]) && args[0].level) {
			return _.isEmpty(fields) ? args[0] : _.extend(args[0], fields);
		}
		let result = { subSystem: this.subSystem };
		if (!_.isEmpty(fields)) _.extend(result, fields);
		let firstNonErrorObj = true;
		let firstError = true;
		for (let arg of args) {
			if (_.isString(arg)) {
				if (arg.match(/^silly|debug|verbose|info|warn|error$/i)) {
					if (!result.level) result.level = arg.toLowerCase();
					else result.message = result.message ? `${arg.toLowerCase()}: ${result.message}` : `${arg.toLowerCase()}: `;
				} else {
				...
				if (arg instanceof Error) {
					if (firstError) {
						if (arg.stack) {
							...
							_.extend(result.details, { stack: arg.stack }) 
						}
						delete arg.stack;
