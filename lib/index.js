if (global.__ZS_LOGGER) {
	...
	*/
	...
	function setLogger(newLogger) {
		logger = newLogger;
		let protoProps = Object.getOwnPropertyNames(Object.getPrototypeOf(logger.__proto__)); /*eslint "no-proto": 0*/
		_.forEach(protoProps, (key) => {
			if (_.isFunction(logger[key])
				&& _.includes([ 'log', 'silly', 'debug', 'verbose', 'info', 'warn', 'error' ], key)
			) {
